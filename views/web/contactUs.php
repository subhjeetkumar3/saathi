...
<h1 class="name post-title entry-title" itemprop="itemReviewed" itemscope="" itemtype="http://schema.org/Thing"><span itemprop="name">Contact Us</span></h1>
<p class="hr_line"></p>
<div class="container">
<div class="send_enquiry"> 
<h3>Send an Enquiry</h3>

	<form action="<?php echo base_url()?>homeweb/getContactRequestInfo" method='POST'>

    <?php if(!$this->session->userdata('validated') || $this->session->userdata('userType') == 'admin'){?> 
     <label for="name"> Your Name*</label>
    <?php }?>
    <input type="<?php if(!$this->session->userdata('validated') || $this->session->userdata('userType') == 'admin'){echo 'text';}else{echo 'hidden';}?>" id="name" value="<?php if($this->session->userdata('validated') && $this->session->userdata('userType') != 'admin'){echo $this->session->userdata('userName');}?>" name="name">

 <?php if(!$this->session->userdata('validated') || $this->session->userdata('userType') == 'admin'){?>
    <label for="email"> Your Email*</label>
  <?php }?>  
    <input type="<?php if(!$this->session->userdata('validated') || $this->session->userdata('userType') == 'admin'){echo 'text';}else{echo 'hidden';}?>" id="email" value="<?php if($this->session->userdata('validated') && $this->session->userdata('userType') != 'admin'){echo $this->session->userdata('email');}?>" name="email" >
...
<?php if(!$this->session->userdata('validated') || $this->session->userdata('userType') == 'admin'){?>     
    <input type="text" id="" disabled="" name="" placeholder="+91-" style="width:10%;font-weight: bold;color: #000000;" class="form-control" onkeypress="return isNumberKey(event)" required="" >
    <?php }?>

    <input type="<?php if(!$this->session->userdata('validated') || $this->session->userdata('userType') == 'admin'){echo 'text';}else{echo 'hidden';}?>" id="phone" value="<?php if($this->session->userdata('validated') && $this->session->userdata('userType') != 'admin'){echo $this->session->userdata('mobile');}?>" name="mobile" maxlength="10" style="width: 89%;" onkeypress="return isNumberKey(event)">
    </div>
     <p style="display: none;color: red" id="spanMobile">Mobile no should have atleast 10 digits</p>
    <label for="subject">Subject</label>
    <select id="subject" name="subject">
      <option value="Gender">Gender, Sex & Sexuality</option>
      <option value="human">Human Rights & Legal Issues</option>
      <option value="Social">Social & Lifestyle</option>
       <option value="Livelihood">Livelihood Options</option>

      <option value="General">General & Sexual Health</option>

      <option value="HIV">HIV & AIDS</option>
      <option value="Volunteering">Volunteering with Us</option>

      <option value="Approval">Approval to Quote from Sahay website</option>
    ...
    (function($){
        ...
        $('form').submit(function(){
            if($('#email').val() == '' && $('#phone').val() == '')
            {
                ...
                alert('you need to enter either Email or Mobile No to move forward');
            }   
        });
