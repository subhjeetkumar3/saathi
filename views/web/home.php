	...
	public function uploadExcelUser(){
		if($_FILES['importExcel']['name']){		
			...
			if(move_uploaded_file($_FILES["importExcel"]["tmp_name"],$file_path)){
				...
				foreach($arr_data as $row){
					if(trim($row['F']) != '' && trim($row['G']) != '' && trim($row['H']) != '' && trim($row['I']) != '' && trim($row['J']) != '' && trim($row['K']) != '' && trim($row['L']) != '' && trim($row['M']) != '' && trim($row['N']) != '' ){
						$len = strlen(trim($row['K']));
					
					  	if($len == 10)
						  {
							$checkStateDistrict = $this->rolemaster->checkStateDistrict(trim($row['M']),trim($row['N']));
							// echo '<pre>';print_r($checkStateDistrict);exit;
							if($checkStateDistrict){
								$checkStateDistrict1 = $this->rolemaster->checkStateDistrict(trim($row['Z']),trim($row['AA']));
							
								if($checkStateDistrict1)
							  {	

							     $userMobile = $this->rolemaster->checkUserMobile(trim($row['K']));
							     ...
							     if(empty($userMobile))
							     {
							     	...
							     	if(!empty(trim($row['A'])))
							     	{
							     	  ...
							     	  //  $insert['registerFromDevice'] = trim($row['D']);
									    ...
									    $insert['mobileNo'] = '+91'.trim($row['K']);
									    $insert['address'] =  trim($row['L']);
									    $insert['educationalLevel'] = trim($row['O']);
									    $insert['occupation'] = trim($row['P']);
									    $insert['occupation_other'] = trim($row['Q']);
									    $insert['hrg'] = trim($row['R']);
									    $insert['arg'] = trim($row['S']);
									
									    $insert['monthlyIncome'] = trim($row['T']); 	
									    $insert['maritalStatus'] = trim($row['U']);
									    $insert['maritalStatus_other'] = trim($row['V']);
									    $insert['male_children'] = trim($row['W']);
									    $insert['female_children'] = trim($row['X']);
									    $insert['total_children'] = trim($row['Y']);
                                    ...
                                    {
                                    	 $insert['artDate'] = date('Y-m-d',PHPExcel_Shared_Date::ExcelToPHP(trim($row['AX'])));
								...
								$error[$j]['error'] = 'Native State or Native District not Match';
								  $error[$j]['registeredOn'] = trim($row['A']);
							     	  ...
							     	  //  $error[$j]['registerFromDevice'] = trim($row['D']);
									    ...
									    $error[$j]['mobileNo'] = trim($row['K']);
									    $error[$j]['address'] =  trim($row['L']);
									    $error[$j]['educationalLevel'] = trim($row['O']);
									    $error[$j]['occupation'] = trim($row['P']);
									    $error[$j]['occupation_other'] = trim($row['Q']);
									    $error[$j]['hrg'] = trim($row['R']);
									    $error[$j]['arg'] = trim($row['S']);
									
									    $error[$j]['monthlyIncome'] = trim($row['T']); 	
									    $error[$j]['maritalStatus'] = trim($row['U']);
									    $error[$j]['maritalStatus_other'] = trim($row['V']);
									    $error[$j]['male_children'] = trim($row['W']);
									    $error[$j]['female_children'] = trim($row['X']);
									    $error[$j]['total_children'] = trim($row['Y']);
									    $error[$j]['state'] = trim($row['Z']);
									    $error[$j]['districtId'] = trim($row['AA']);
									    $error[$j]['addressState'] = trim($row['M']);
									    $error[$j]['addressDistrict'] = trim($row['N']);
 ...
 public function downloadStock()
  {
     ...
     public function downloadUserData()
     {
		...
		$header = array('Upload ART Green Card scan / photo','Upload ICTC test report scan','Upload Referral Slip');
		...
		foreach ($result as $obj => $val) {
            $this->excel->setActiveSheetIndex(0);
            /*$this->excel->getActiveSheet()->setCellValue('A' . $i, $val['userUniqueId']);
            $this->excel->getActiveSheet()->setCellValue('B' . $i, $val['userName']);
            $this->excel->getActiveSheet()->setCellValue('C' . $i, $val['name']);
            $this->excel->getActiveSheet()->setCellValue('D' . $i, $val['nameAlias']);
             $this->excel->getActiveSheet()->setCellValue('E' . $i, $val['dob']);
            $this->excel->getActiveSheet()->setCellValue('F' . $i, $val['mobileNo']);
            $this->excel->getActiveSheet()->setCellValue('G' . $i, $val['address']);
            $this->excel->getActiveSheet()->setCellValue('H' . $i, $val['addressState']);
            $this->excel->getActiveSheet()->setCellValue('I' . $i, $val['addressDistrict']);
            $this->excel->getActiveSheet()->setCellValue('J' . $i, $val['educationalLevel']);
            $this->excel->getActiveSheet()->setCellValue('K' . $i, $val['occupation']);
            $this->excel->getActiveSheet()->setCellValue('L' . $i, $val['occupation_other']);
            $this->excel->getActiveSheet()->setCellValue('M' . $i, $val['monthlyIncome']);
             $this->excel->getActiveSheet()->setCellValue('N' . $i, $val['maritalStatus']);
            $this->excel->getActiveSheet()->setCellValue('O' . $i, $val['maritalStatus_other']);
            $this->excel->getActiveSheet()->setCellValue('P' . $i, $val['male_children']);
            $this->excel->getActiveSheet()->setCellValue('Q' . $i, $val['female_children']);
            $this->excel->getActiveSheet()->setCellValue('R' . $i, $val['total_children']);
            $this->excel->getActiveSheet()->setCellValue('Q' . $i, $val['state']);
             $this->excel->getActiveSheet()->setCellValue('R' . $i, $val['districtId']);
            ...
            $this->excel->getActiveSheet()->setCellValue('T' . $i, $val['secondaryIdentity_other']); 
             $this->excel->getActiveSheet()->setCellValue('U' . $i, $val['sexualBehaviour']);
            $this->excel->getActiveSheet()->setCellValue('V' . $i, $val['multipleSexPartner']);
            $this->excel->getActiveSheet()->setCellValue('W' . $i, $val['sought']);
             $this->excel->getActiveSheet()->setCellValue('X' . $i, $val['prefferedGender']);
            $this->excel->getActiveSheet()->setCellValue('Y' . $i, $val['prefferedSexualAct']);
            $this->excel->getActiveSheet()->setCellValue('Z' . $i, $val['substanceUse']);
             $this->excel->getActiveSheet()->setCellValue('AA' . $i, $val['testHiv']);
            $this->excel->getActiveSheet()->setCellValue('AB' . $i, $val['hivTestResult']);
            $this->excel->getActiveSheet()->setCellValue('AC' . $i, $val['pastHivReport']);
            $this->excel->getActiveSheet()->setCellValue('AD' . $i, $val['remark']);
            $this->excel->getActiveSheet()->setCellValue('AE' . $i, $val['createdDate']);*/
           /* $this->excel->getActiveSheet()->setCellValue('AF' . $i, $val['artUpload']);
              $this->excel->getActiveSheet()->setCellValue('AG' . $i, $val['ictcReportScan']);
            $this->excel->getActiveSheet()->setCellValue('AH' . $i ,$val['referralSlip']);*/
		 ...
		 { $objDrawing2 = new PHPExcel_Worksheet_Drawing();
		 			...
		 			$objDrawing2->setCoordinates('C'.$i);
...
$data['cost_of_transporting'] = number_format($this->input->post('transportingCost'), 2, '.', '');
  ...
  /*  public function addPhoto()
   ...
   public function getPositiveReport()
   {
    ...
    $header = array('Date of Registration','Place of Registration','SAATHII Registration No.','Name of the Client','Address','District','State','Age','Gender','Identity','If others then specify','Mode of Contact','Date of Out-referral to SA-ICTC','Place of SA-ICTC Referred','ICTC - PID Number','Date of HIV Confirmation Test','Linked to ART','ART Registration Date','ART Registration Number','Baseline CD4 Count','REMARKS');
