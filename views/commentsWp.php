		   ...
		   <?php }?>
			<div class="row">
				<div class="col-lg-12">
					...
					<div id="tab-2" class="tab-pane <?php if(empty($id)){ echo 'active'; } ?> ">
						   ...
						   <div class="ibox-title">
                            ...
                            <div class="ibox-tools">
                            	<a class="btn btn-primary" href="<?php echo base_url()?>index.php/home/downloadCommentdata">
                            		<i class="fa fa-download"></i> Comments Data
                            	</a>
                             ...
                             <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover">
									...
									<tr id="row<?php echo $comments['comment_ID']?>">
										...
										<td><?php  
                    if($comments['anotherStatus'] == 'reject')
                    {
                      echo 'Rejected';
                    }
                    elseif ($comments['anotherStatus'] == 'approve') 
                    {
                      echo 'Approved';
                    }else
                    {
                    ...
                    ?></td>
										<td><?php echo date('d M Y h:i a',strtotime($comments['comment_date']));?></td>
										<td>
											<!-- <span class="btn-white btn btn-xs">Edit</span>
											<span class="btn-white btn btn-xs">Delete</span> -->
										<?php if($comments['anotherStatus']=='reject'){ ?>
											<span class="btn-danger btn btn-xs" onclick="changeCommentStatus('0','<?php echo $comments['comment_ID']?>','pending')">
												Unapprove
											</span>
                      <span class="btn-success btn btn-xs" onclick="changeCommentStatus('1','<?php echo $comments['comment_ID']?>','approve')">
                        Approve
                      </span>
										<?php }  
                     else if ($comments['anotherStatus']=='approve') { ?>
											<span class="btn-danger btn btn-xs" onclick="changeCommentStatus('0','<?php echo $comments['comment_ID']?>','pending')">
												Unapprove
											</span>
                      <span class="btn-warning btn btn-xs" onclick="changeCommentStatus('0','<?php echo $comments['comment_ID']?>','reject')">
                        Reject
                      </span>
										<?php }else{ ?>
                      <span class="btn-warning btn btn-xs" onclick="changeCommentStatus('0','<?php echo $comments['comment_ID']?>','reject')">
...
<!-- 						<div id="tab-1" class="tab-pane">
    ...
    <div class="modal inmodal" id="changeStatusResultModal" role="dialog" data-backdrop="static" data-keyboard="false">
       <div class="modal-dialog">
        ...
        function changeCommentStatus(status,commentId,anotherStatus){
        	$.ajax({

    		 type: "POST",
                url: "changeCommentStatusWp",
                data: {status:status,commentId:commentId,anotherStatus:anotherStatus},
