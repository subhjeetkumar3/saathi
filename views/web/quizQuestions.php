				...
				<div class="panel panel-default" id="question<?php echo $i; ?>" >
					...
					<?php $j=1; foreach($name['options'] as $options) { ?>
						<div>
							...
							<label> <input type="checkbox" data-ques="<?php echo $i.$j; ?>" value="<?php echo $options['optionId']; ?>" id="optionsRadios1"  name="options[<?php echo $name['quizQuestionId']; ?>][]" class="question<?php echo $i; ?>" > <?php echo $options['optionName']; ?></label> 
						</div>
						<?php $j++; } ?>    
					</div>
...
<!-------- model for no answer submition --------->
		<div class="hover_bkgr_fricc" id="noAnswer">
		    <span class="helper"></span>
		    <div>			
				<div id="infoDIv">
						<div style="font-weight: bold;margin-top: 10px;color:red;" > 
			...
			var html = '';
	         ...
	         $.ajax({
	         	...
	         	success:function(data){
					...
					html = '<div class="corretAns"><span>Correct Answer(s) : </span></br>';
					...
					html += '</div><div style="color: black;margin-top: 10px;"> < '+result[0]['AdditionalInfoIncaseOfCorrectAnswer']+' > </div>';
						...
						html += '</div><div style="color: black;margin-top: 10px;"> < '+result[0]['AdditionalInfoInCaseOfWrongAnswer']+' > </div>';
					}
